...
<div class="pagetitle">
    ...
    <nav>
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">Pihak</li>
        </ol>
    </nav>
</div><!-- End Page Title -->
@endsection
...
<div class="col-lg-12">
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center mb-3">
            <h3 class="card-title d-flex">Data Pihak</h3>
                ...
                <div class="modal-body">
                    ...
                    <div class="row">
                        ...
                        <div class="col-12">
                            ...
                            <div class="input-group has-validation">
                                <textarea type="text" name="alamat" class="form-control" rows="5" id="alamat" required></textarea>
...
<script type="text/javascript">
    ...
    $("#kabupaten_id").select2({
        ...
        dropdownParent: $("#kabupaten_id").parent(),
        ajax: {
            url: "{{  route('kabupaten-search') }}",
            dataType: 'json',
            delay: 250,
            type:'post',
    ...
    function fillform(dt) {
        ...
        //$('#kabupaten_id').val(dt.kabupaten_id).trigger('change');
    }
    ...
    $(document).on("click", ".btn-ganti", function() {
        ...
        appAjax("{{ route('pihak-update') }}", formVal).done(function(vRet) {
            if (vRet.status) {
                var myModal = new bootstrap.Modal(document.getElementById('modal-form-web'), {
                    backdrop: 'static',
                    keyboard: false,
    ...
    function hapus(idTerpilih) {
        ...
        if (idTerpilih.length > 0 && confirm("apakah anda yakin?")) {
            appAjax("{{ route('pihak-delete') }}", formVal).done(function(vRet) {
                if (vRet.status) {
                    reloadTable();
                }
                showmymessage(vRet.messages, vRet.status);
    ...
    $("#fweb").submit(function(e) {
        ...
        if (isValid) {
            appAjax('{{ route("pihak-create") }}', formVal).done(function(vRet) {
                if (vRet.status) {
                    if (vRet.insert)
                        resetform();
                    reloadTable();
