...
<div class="col-lg-12">
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center mb-3">            
            <h3 class="card-title d-flex">Data Jenis</h3>
...
@section("scriptJs")
    ...
    <script type="text/javascript">
        ...
        $(document).on("click",".btn-ganti",function(){
            ...
            appAjax("{{ route('jenis-update') }}", formVal).done(function(vRet) {
                 if(vRet.status){
                    var myModal = new bootstrap.Modal(document.getElementById('modal-form-web'), {
                        backdrop: 'static',
                        keyboard: false,
        ...
        function hapus(idTerpilih){
            ...
            if(idTerpilih.length > 0 && confirm("apakah anda yakin?")){
                appAjax("{{ route('jenis-delete') }}", formVal).done(function(vRet) {
                    if(vRet.status){
                        reloadTable();
                    }
                    showmymessage(vRet.messages,vRet.status);
        ...
        $("#fweb").submit(function(e) {
            ...
            if(isValid){
                appAjax('{{ route("jenis-create") }}', formVal).done(function(vRet) {
                    if(vRet.status){
                        if(vRet.insert)
                            resetform();
                        reloadTable();
